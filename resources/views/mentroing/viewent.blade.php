                    ...
                    <strong>Mentoring Session Details</strong>
                        ...
                        <form class="row g-3" role="form" method="POST" action="{{route('updatementoring')}}" enctype="multipart/form-data">
                            ...
                            <div class="form-group col-md-5">
                                ...
                                <option value="Option one" selected>-- Selec one --</option>
                                    <?php $cohorts = App\Models\Funding::all();
                                     foreach($cohorts as $j):
                                        ?>
                                       <option value="{{$j->id}}" <?php if($app->EligibleCohorts==$j->id) { ?> selected <?php } ?>>{{$j->title}}</option>
                                      <?php endforeach ?>
                                </select>
                                </select>
                            </div>
                            ...
                            <div class="modal-footer">
                                <a  href="{{ url()->previous() }}"><button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">Back</button></a>
                            </div>
                        </form>
                    </div>
                </div>
