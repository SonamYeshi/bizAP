            ...
            <thead class="thead-light">
               ...
               <?php $i = 1;
                ...
                <tr>
                    ...
                    <td style="font-size: 14px;"><?php echo App\Models\Funding::where('id', $t->fundid)->value('opencohort');?>&nbsp;<?php echo App\Models\Funding::where('id', $t->fundid)->value('opencohortno');?></td>
                    ...
                    <a href="{{route('delete_fsl',['id'=>$t->id])}}" onclick='return confirm("Are you sure to Delete ?")' class="btn btn-outline-danger btn-sm">Delete</a>
                    </td>
                    </tr>
                    <?php $i++;
                    endforeach;
             ...
             <input type="hidden" name="hidden_view" id="hidden_view" value="{{url('fslpanel/edit')}}">
        </div>
    </div>
  </div>
...
<div class="modal-content">
      ...
      <form class="form-horizontal" role="form" method="POST" action="{{ route('fshortpaneladd') }}">
      {{ csrf_field() }}
      <div class='form-group'>
        <label for='type' class='col-xs-4 control-label'>Training<font color="red">*</font></label>
          <select class='form-control' id="training" name="training" required="required">
           ...
           ?>
           <option value="{{$tr->id}}">{{$tr->opencohort}}&nbsp;{{ $tr->opencohortno}}</option>
          <?php endforeach ?>
          </select>
        <div class='col-xs-6'>
        </div>
...
<div class="modal-content">
        ...
        <div class="modal-body">
          <form class="form-horizontal" role="form" method="POST" action="{{ route('fupdate_slmember') }}">
          {{ csrf_field() }}
          <div class='form-group'>
        <label for='type' class='col-xs-4 control-label'>Training<font color="red">*</font></label>
