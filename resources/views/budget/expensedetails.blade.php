  ...
  <div class="container">
   <div class="row">
        ...
        @include('flash-message')
            <form id='myform' action="{{ route('expenseyear') }}" method="post">
            {{ csrf_field() }}
            ...
            <table id="example" class="table table-bordered table-sm table-striped">
               ...
               <?php $i = 1; foreach($ExpenseDetails as $bh): ?>
                <tr>
                    ...
                    <td><?php echo number_format($bh->BudgetAmount, 2); ?></td>
                    <td><?php
                     $ExpenseDetails=App\Models\ExpenseDetails::where('BudgetDetailsID', $bh->id)->get();
                     $total = '0';
                     foreach($ExpenseDetails as $tr):
                        $total = ($total + $tr->ExpenseAmount);
                     endforeach;
                     echo number_format($total, 2);
                     ?>
                    </td>
                    <td><?php
                    echo number_format(($bh->BudgetAmount - $total), 2);
                    ?></td>
                    <td>
                    <a href="{{ route('updateexpensedetails', $bh->id) }}" style="text-decoration:none">Update</a>
                    </td>
                    </tr>
                    <?php $i++; endforeach; ?>
               </tbody>
