                ...
                <div class="container">
                    <div class="row">
                        ...
                        @include('flash-message')
                            ...
                            <form class="row g-3" role="form" method="POST" action="{{ route('insertrefund') }}" enctype="multipart/form-data">
                             {{ csrf_field() }}
                                  <input type="hidden" name="fundid" value="{{$fundid}}" />
                                  <input type="hidden" name="cid" value="{{$cid}}" />
                                  <input type="hidden" name="tfd" value="{{$tfd}}" />
                                  <input type="hidden" name="key" value="{{$key}}" />
                                  <div class="row mb-4">
                                <label for="inputPassword" class="col-sm-2 col-form-label">Refund Amount:<font color="red">*</font></label>
                                <div class="col-sm-8">
                                    <input type="number" class="form-control" name="PaymentAmount" value="" required>
                                </div>
                            </div>
