                ...
                <div class="container">
                    <div class="row">
                        ...
                        <nav class="navbar navbar-light">
                            <strong>Interview score sheet for <font color="blue">{{ $application->name }}</font></strong>
                        </nav>
                        <div id="successMessage">@include('flash-message')</div>
                        <form class="row g-3" role="form" method="POST" action="{{ route('interviewupdatescore') }}" enctype="multipart/form-data" id="interviewScoreForm">
                            {{ csrf_field() }}
                            <input type="hidden" name="appid" value="<?php echo $appid; ?>">
                            <input type="hidden" name="trainingid" value="<?php echo $application->trainingid; ?>">
                            ...
                            <div class="form-group col-md-10">
                                <label for="name" class="form-label"><strong>Interview Scores:&nbsp;</strong></label>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($pannels as $pl)
                                            <tr>
                                                ...
                                                @if(!is_null($application->interview_status))
                                                readOnly
                                                @endif
                                                ></td>
                                        ...
                                        <tr>
                                            ...
                                            <td><a class="bbtn btn-primary btn-sm" href="{{ route('export', $application->trainingid) }}" style="text-decoration: none;">Export Score Sheet</a></td>
                                            <td>
                            ...
                            <div class="modal-footer">
                                @if(is_null($application->interview_status))
                                <button type="submit" class="btn btn-primary btn-sm" onclick='return validateAndConfirm({{$application->interviewdate_id}})'>Save</button>
                                @endif
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal" method="POST" action="{{ route('import') }}" accept-charset="utf-8" enctype="multipart/form-data" >
          ...
          <input type="hidden" name="slstatus" value="{{ $application->shortlist_status }}">
...
function validateAndConfirm(interviewdate_id){
    var isValid = true;
    if(interviewdate_id == null){
        alert("Interview date has not been scheduled.");
        return false;
    }else{
        $('#interviewScoreForm :input[required]').each(function() {
      if (!$(this).val().trim()) {
          isValid = false;
                                        ...
                                        <table id="example" class="table table-bordered table-sm table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input class="form-check-input" type="radio" disabled <?php if ($application->Punctuality == '3') { ?>checked<?php } ?>></td>
