...
class ApplicantController extends Controller
{
    ...
    public function fundreview()
    {
        ...
        $allapplication = DB::table('tbl_fund_request as fr')
        ->join('tb_dhifund_applications as app', 'fr.fundid', '=', 'app.id')
        ->leftjoin('tbl_fundrequest_statuses as frs', 'fr.id', 'frs.fundrequestid')
        ->select('fr.*',
        'app.name as name',
        'app.cid as cid',
        'app.email as email',
        'app.mobileno as mobileno',
        'app.business_name as business_name',
        'app.business_location as business_location',
        'frs.review_status as review_status',
        'frs.approve_status_asd as asd',
        'frs.approved_status_dir as dir',
        'frs.approved_status_ach as ach',
        )->orderBy('fr.id', 'desc')
        ->get();

        $funding = Funding::all();
        return view('fundrequest.reviewlist', compact('allapplication', 'funding'));
    }
    ...
    public function fundreviewsearch(Request $request)
    {
        ...
        return view('fundrequest.reviewlist', compact('allapplication', 'funding'));
    //    return view('fundrequest.reviewlist1', compact('allapplication', 'funding'));
    }
    ...
    public function fundappreview($id)
    {
        $fundappid = FundRequest::where('id', $id)->value('fundid');
        $allapplication = FundingApplication::where('id', $fundappid)->get();
        return view('fundrequest.review', compact('allapplication', 'fundappid', 'id'));
    }

    public function fundappreviewasd($id)
    {
        $fundappid = FundRequest::where('id', $id)->value('fundid');
        $allapplication = FundingApplication::where('id', $fundappid)->get();
        return view('fundrequest.viewasd', compact('allapplication', 'fundappid', 'id'));
    }

    public function updatefundreview(Request $request)
    {
        $key = $request->key;
        $NameofEntrepreneur = $request->NameofEntrepreneur;
        ...
        $fund = new FundingApplication;
             $fund::where('id', $fundappid)
            ->update([
                'name' => $NameofEntrepreneur,
                'business_name' => $BusinessName,
                'business_licence_no' => $LicenseNo,
            ...
            $Tranche = $request->Tranche;
            $Usage = $request->Usage;
            $Tobepaid = $request->Tobepaid;

             $fundr = new FundRequest;
             $fundr::where('id', $id)
            ->update([
                'tranche' => $Tranche,
                'usage' => $Usage,
    ...
    public function postreviewasd(Request $request)
    {
        ...
        if($role == '12'){
	     ...
	     $appid =  FundRequest::where('id', $request->fundid)->value('fundid');
             $cid =  FundingApplication::where('id', $appid)->value('cid');
             $Emails = User::where('role_id', '1')->get();
             $business_name = $request->asd_remarks;
             $name = FundingApplication::where('cid', $cid)->sum('name');
             foreach ($Emails as $e)
             {
              Mail::to($e->email)->send(new ApproveDirMail($business_name, $name));
             }
            }
         $fundrequest = new FundRequest;
         if($role == '12'){
    ...
    public function disbursement()
    {
        ...
        $allapplication = DB::table('tbl_fund_request as fr')
        ->join('tb_dhifund_applications as app', 'fr.fundid', '=', 'app.id')
        ->leftjoin('tbl_disbursement_docs as doc', 'doc.appid', 'app.id')
        ->where('fr.review', 1)
        ->where('fr.approve_ach', 1)
        ->where('fr.approve_asd', 1)
        ->where('fr.approve_dir', 1)
        ->select('fr.*',
        'app.name as name',
        'app.cid as cid',
        'app.cohortopen as cohortopen',
        'app.mobileno as mobileno',
        'app.business_name as business_name',
        'app.cohortopenno as cohortopenno',
        'app.actual_disbursed as actual_disbursed',
        ...
        return view('fundrequest.disbursementlist', compact('allapplication', 'key', 'funding'));
    }
       ...
       $fundid = $request->fundid;
        ...
        if ($request->disbursement != 0) {
            foreach ($request->disbursement as $file) {
                $filename = $file->getClientOriginalName();
                $relative_path = '/uploads/disbursementdocs/'.$cid;
	...
	$dir = User::where('role_id', '12')->value('name');
        $ad = User::where('role_id', '5')->value('name');
        $allapplication = FundingApplication::where('id', $fundid)->get();
        $fundapplication = new FundingApplication;

        $fundapplication::where('id', $fundid)->update(['total_disbursed' => '500000', 'disbursed_date' => date('Y-m-d'), 'disbursement' => '1']);
        return view('fundrequest.reviewdisbursement', compact('allapplication', 'id', 'dir', 'accounthead', 'ad', 'fundid', 'key'));
    }
	...
	$dir = User::where('role_id', '12')->value('name');
        $ad = User::where('role_id', '5')->value('name');
        $allapplication = FundingApplication::where('id', $fundid)->get();
        $pdf = PDF::loadView('fundrequest.reviewdisbursementpdf', compact('allapplication', 'id', 'dir', 'accounthead', 'ad', 'fundid'));
    ...
    public function allrepayment($fundid, $cid, $tfd)
    {
	    ...
	    $effectivedate = ContractDateTime::where('cid', $cid)->value('effective_date');
            return view('repayment.allpayment', compact('fundid', 'cid', 'tfd', 'allapplication', 'effectivedate'));
    }
    ...
    public function updatepayment($fundid, $cid, $tfd)
    {
        ...
        $intres_rate = Funding::where('opencohort', $co)->where('opencohortno', $cono)->value('emiintres_rate');
    ...
    public function insertpayment(Request $request)
    {
        ...
        $co = FundingApplication::where('id', $fundid)->sum('cohortopen');
        $cono = FundingApplication::where('id', $fundid)->sum('cohortopenno');
        $emiintrest = Funding::where('opencohort', $co)->where('opencohortno', $cono)->value('emiintres_rate');
        $tenure = Funding::where('opencohort', $co)->where('opencohortno', $cono)->value('tenure');
       ...
       //case1 early date
        ...
        $ed1=date('Y-m-d', strtotime('+1 year', strtotime($ed)) );


        $pd = date('Y-m-d',strtotime($payment_date));
        $date1 = new DateTime($ed1);
        $date2 = new DateTime($pd);
        $interval = date_diff($date1, $date2);
        $edays = $interval->format('%a');
        $fine = '0';

        $intrest1=round(($interest/100*$principalamount*($edays/365)), 0);
        $principle1 = (($principalamount + $intrest1) - $emi);

            //case2 later date
            $date1 = date('Y-m-d',strtotime($pd));
            $date1 = new DateTime($date1);
            $dyear = date('y', strtotime($pd));
            $dmonth = date('m', strtotime($pd));
            if($dmonth =='02'){$dday = '28';}
            if($dmonth =='01'|| $dmonth =='03' || $dmonth =='05' || $dmonth =='07' || $dmonth =='08' || $dmonth =='10' || $dmonth =='12'){$dday = '31';}
            if($dmonth =='04'|| $dmonth =='06' || $dmonth =='09' || $dmonth =='11'){$dday = '30';}
            $date = $dyear.'-'.$dmonth.'-'.$dday;
            $date2 = date("Y-m-d",strtotime($date));
            $date2 = new DateTime($date2);
           $interval = date_diff($date1, $date2);
           $delaydays = $interval->format('%a');
           $ldays = $interval->format('%a')+1;

           $intrest2=round(($interest/100*$principle1*($ldays/365)), 0);
           $principle2 = ($principle1 + $intrest2);
           $ad = ($intrest1 + $intrest2);
           $pr = ($emi-$ad);
    ...
    $payment->administrative_fee = $ad;
    $payment->principal_repayment = $pr;
    $payment->emi_amount = $request->PaymentAmount;
    $payment->closing_balance = $principle2;
    ...
    $payment->due_date = $date2;
    $payment->review_status = '0';
    $payment->penalty = '0';
    ...
    else
    {
        ...
        $lastpaymentdate = Repayments::where('cid', $cid)->orderBy('id', 'desc')->value('due_date'); echo "<br>";
        $administrativefee = round(($interest/100*$principalamount)/12, 2);
        $principalrepayment = round($emi_amount - $administrativefee, 2);
        $closingbalance = round($principalamount - $principalrepayment, 2);
       //case1 early date

        $ed=date('Y-m-d', strtotime($payment_date));
        $dyear = date('y', strtotime($ed));
        $dmonth = date('m', strtotime($ed));
        $dday = "01";
        $edd = $dyear.'-'.$dmonth.'-'.$dday;
        $ed1 = date("Y-m-d",strtotime($edd));
        ...
        $principle1 = (($principalamount + $intrest1) - $emi);
           ...
           $lmonth = date('m', strtotime($lastpaymentdate));
           if($lmonth == '12')
           {$pmonth = 01;
            $lyear = date('y', strtotime($lastpaymentdate));
            $lyear = $lyear + 1;
           }
           else{$pmonth = $lmonth + 01;
            $lyear = date('y', strtotime($lastpaymentdate));
           }

            if($pmonth =='02'){$dday = '28';}
            if($pmonth =='01'|| $pmonth =='03' || $pmonth =='05' || $pmonth =='07' || $pmonth =='08' || $pmonth =='10' || $pmonth =='12'){$dday = '31';}
            if($pmonth =='04'|| $pmonth =='06' || $pmonth =='09' || $pmonth =='11'){$dday = '30';}
            $date = $lyear.'-'.$pmonth.'-'.$dday;
            $date1 = date('Y-m-d',strtotime($payment_date));
            $date1 = new DateTime($date1);
            $date2 = date("Y-m-d",strtotime($date));
            $date2 = new DateTime($date2);
            $interval = date_diff($date2, $date1);
            $delaydays = $interval->format('%r%a');
            $pm = $interval->format('%r');
       if( $pm != '-') {
        $duedate = date('Y-m-d', strtotime($lastpaymentdate));
        $fine = round((($emi * 0.02 * $delaydays)/365), 2);
            $delay = new DelayCount;
            $delay->cid = $cid;
            $delay->duedate = $duedate;
            $delay->paymentdate = $pd;
    ...
    {   $key = "0";
        ...
        $funding = Funding::all();
        return view('repayment.dhi', compact('allapplication', 'funding'));
    }
    ...
    public function allrepaymentdhi($fundid, $cid){
	    $allapplication = Repayments::where('cid', $cid)->orderBy('id', 'asc')->get();
	    $name = FundingApplication::where('id', $fundid)->value('name');
	    $effectivedate = ContractDateTime::where('cid', $cid)->value('effective_date');
        return view('repayment.allpaymentdhi', compact('fundid', 'cid', 'allapplication', 'name', 'effectivedate'));
    }

    public function updatepaymentdhi($fundid, $cid)
    {
        $total_disbursed = FundingApplication::where('cid', $cid)->sum('actual_disbursed');
        $co = FundingApplication::where('id', $fundid)->sum('cohortopen');
        $cono = FundingApplication::where('id', $fundid)->sum('cohortopenno');
       ...
       return view('repayment.updatepaymentdhi', compact('fundid', 'cid', 'emi', 'penalty'));
    }

    public function refundpaymentdhi($fundid, $cid, $tfd)
    {
        $key =0;
	...
	/*if($request->mode == 'inperson'){
        ...
        {
         Mail::to($myEmail)->send(new SendSiteVisit_1($request->fundid, $request->svdate, $request->svtime, $request->Agencda));
        }*/
